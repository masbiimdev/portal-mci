            ...
            <div class="card p-3">
                <div class="mb-3">
                    ...
                    <input type="text" id="material_code" name="material_code"
                        class="form-control @error('material_code') is-invalid @enderror" readonly
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <select id="valve_id" name="valve_id[]" multiple
                            class="form-control select2 @error('valve_id') is-invalid @enderror">
                            @foreach ($valves as $v)
                                <option value="{{ $v->id }}" @if (in_array($v->id, old('valve_id', $selectedValves ?? []))) selected @endif>
                                    {{ $v->valve_name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select id="spare_part_id" name="spare_part_id"
                            ...
                            @foreach ($spareParts as $sp)
                                <option value="{{ $sp->id }}"
                                    {{ old('spare_part_id', $material->spare_part_id) == $sp->id ? 'selected' : '' }}>
                                    {{ $sp->spare_part_name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="mb-3">
                    ...
                    <select id="rack_id" name="rack_id" class="form-control @error('rack_id') is-invalid @enderror">
                        ...
                        @foreach ($racks as $r)
                            <option value="{{ $r->id }}"
                                {{ old('rack_id', $material->rack_id) == $r->id ? 'selected' : '' }}>
